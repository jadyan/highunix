...
template <typename T>
Rational2<T> doMultiply(const Rational2<T> &lhs, const Rational2<T> &rhs) {
...
class Rational2 {
	public:
		...
		friend Rational2 operator*(const Rational2 &lhs, const Rational2 &rhs) 
		{
			cout<<"Rational2 相乘"<<endl;							   
		...
		Rational2(const T &numerator = 0,
				const T &denominator = 1) {
			cout<<"构造 "<<numerator << " "<< denominator<<endl;
...
void Test2() {
	...
	Rational2<int> result = one_half * 2;      //失败，1隐式转换为 Rational<T>失败
}
