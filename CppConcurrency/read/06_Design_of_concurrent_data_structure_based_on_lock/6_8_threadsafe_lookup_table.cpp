...
// #endif
 ...
 int main()
 {
    ...
    //threadsafe_lookup_table<int,int> x ;
    x.add_or_update_mapping(1,11);
    x.add_or_update_mapping(18,27);

    std::cout<<x.value_for(1)<<std::endl;
    std::cout<<x.value_for(18)<<std::endl;


    //threadsafe_lookup_table<std::string, std::string,std::hash<std::string>> my_map ;
    threadsafe_lookup_table<std::string, std::string> my_map ;
    my_map.add_or_update_mapping("an","nuonuo");
    std::cout<<my_map.value_for("an")<<std::endl;
#if 0
    std::map<std::string, std::string> strmap;
    strmap = my_map.get_map();
