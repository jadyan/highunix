...
//F_LOCK 和 F_TLOCK 请求仅在采取的操作上有所差异（如果资源不可用）。
//如果区域已被其他进程锁定，F_LOCK 将使调用进程进入休眠状态，直到该资源可用，而 F_TLOCK 则会返回[EACCES]错误。
//F_TEST 用于检测在指定的区域中是否存在其他进程的锁定。如果该区域被锁定，lockf()将返回 -1，否则返回0；在这种情况下，errno 设置为[EACCES]。
//F_LOCK 和 F_TLOCK 都用于锁定文件的某个区域（如果该区域可用）。F_ULOCK 用于删除文件区域的锁定。
//③len是要锁定或解锁的连续字节数。
...
/*
	...
	https://blog.csdn.net/babyfans/article/details/6109106
	page 467
*/
...
int main(){
    ...
    //if ((fd = open("./popen.c.readme", O_RDWR|O_WRONLY)) < 0)
   if ((fd = open("/dev/zero", O_RDWR|O_CREAT)) < 0)   //为通过此函数调用建立锁定，文件描述符必须使用只写权限（O_WRONLY）或读写权限（O_RDWR）打开
   {
       printf("open error\n");
   }
    //fd = open("./add2", O_WRONLY|O_CREAT, S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH);
    printf("xxxxx\n");
    int ret = lockf(fd,F_LOCK,0);
    printf("end = %d\n",ret);
    sprintf(outpipe2,"Child process 2 is sending a message!\n");
    printf("end2\n");
    write(fd,outpipe2,strlen(outpipe2));
    printf("end3\n");
    sleep(10);
